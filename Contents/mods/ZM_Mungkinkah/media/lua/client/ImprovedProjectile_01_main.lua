	...
	for iv, v in pairs(projTable) do
		...
		if v[4] then
			...
			if zombieNum > 0 then
              ...
              local md = zombieTable[i][1]:getOutfitName() or "Random"
              local isElite = false
              local isTank = false
              if md == "ArmyCamoGreen" then
                  isElite = true
              end
              if md == "Dreadnought1" or md == "Dreadnought2" then
                  isTank = true
              end

              if (isElite) then
                  hitDamage = hitDamage * 0.05
              end

              if (isTank) then
                  hitDamage = hitDamage * 0.02
              end
							zombieTable[i][1]:setHealth(zombieTable[i][1]:getHealth() - hitDamage) -- lagi disini
